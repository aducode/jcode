...
#		self.file = open(path,'rb')
	def __str__(self):
		if self.parsed:
			for i in xrange(1,len(self.constant_pool)):
				print '%d-------%s' % (i, self.constant_pool[i])
	...
	def parseConstantInfo(self):
		...
		for i in xrange(1,constant_pool_count):
			...
			constantInfo = constant.Constants.createConstant(tag,[self.file.read(step) for step in constant.Constants.nextStep(tag)])
	...
	def parseMethods(self):
		...
		for i in xrange(methods_count):
			...
			attribute_count = struct.unpack('>H', self.file.read(2))[0]
			if attribute_count and attribute_count>0:
				method['attributes']=self._get_attributes(attribute_count,True)
	...
	def _get_attributes(self, attribute_count, method_attribute=False):
		attributes = []
		if attribute_count and attribute_count>0:
			for j in xrange(attribute_count):
				...
				assert isinstance(attribute_name_utf8, constant.ConstantUtf8)
				...
				attribute['value']=raw_value
				if not method_attribute and data_length == 2:
